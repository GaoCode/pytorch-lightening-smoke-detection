...
def generate_fire_to_images(raw_data_path):
    ...
    fire_to_images = {}
    all_fires = [
        folder.stem for folder in filter(Path.is_dir, raw_data_path.iterdir())
    ]
    ...
    for fire in all_fires:
        ...
        images = [
            get_image_name(str(item))
            for item in (raw_data_path / fire).glob("*.jpg")
        ]
        if len(images) > 0:
...
def shorten_time_range(
    ...
    effective_series_length = (
        effective_series_length + 1
        if optical_flow_path is not None
        else effective_series_length
    )
    ...
    if effective_start > -2400 or time_range[1] < 2400:
        for fire in fires_list:
            ...
            for image in fire_to_images[fire]:
                ...
                if (
                    image_time_index >= effective_start
                    and image_time_index <= time_range[1]
                ):
    ...
    for fire in fire_to_images:
        for i, img in enumerate(fire_to_images[fire]):
            ...
            if series_length != 1 and add_base_flow:
                image_series[img].append(
                    fire_to_images[fire][np.maximum(0, idx - 5)]
                )

            while len(image_series[img]) < series_length:
                image_series[img].insert(
                    int(add_base_flow), fire_to_images[fire][idx]
                )
                if idx != 0:
                    idx -= 1
        ...
        if cur_object.find("name").text == "bp":
            ...
            for cur_poly in cur_object.findall("polygon"):
                ...
                for cur_pt in cur_poly.findall("pt"):
                    cur_poly_pts.append(
                        [int(cur_pt.find("x").text), int(cur_pt.find("y").text)]
                    )
...
class DataAugmentations:
    ...
    def __init__(
        ...
        self.jitter_amount = (
            np.random.randint(tile_dimensions[0])
        ...
        self.should_brightness = (
            np.random.rand() > 0.5 if brightness_contrast_augment else False
        )
    ...
    def __call__(self, img, is_labels=False):
        ...
        self.resize_factor = np.array(self.resize_dimensions) / np.array(
            img.shape[:2]
        )

        # Resize
        img = cv2.resize(
            img, (self.resize_dimensions[1], self.resize_dimensions[0])
        )
        ...
        if self.jitter_amount == 0:
            img = img[-self.crop_height :]  # noqa
        else:
            img = img[
                -(
                    self.crop_height + self.jitter_amount
                ) : -self.jitter_amount  # noqa
            ]
        ...
        if not is_labels:
            ...
            if self.should_color:
                ...
                img[:, :, 0] = np.add(
                    img[:, :, 0], np.random.randn() * 2, casting="unsafe"
                )
    ...
    def process_bboxes(self, gt_bboxes):
        ...
        for i in range(len(gt_bboxes)):
            ...
            bboxes[i][1] = gt_bboxes[i][1] * self.resize_factor[0] - (
                self.resize_dimensions[0]
                - self.crop_height
                - self.jitter_amount
            )
            bboxes[i][2] = gt_bboxes[i][2] * self.resize_factor[1]
            bboxes[i][3] = gt_bboxes[i][3] * self.resize_factor[0] - (
                self.resize_dimensions[0]
                - self.crop_height
                - self.jitter_amount
            )
...
def randomly_sample_tiles(x, labels, num_samples=30):
    ...
    sampled_pos_indices = np.random.choice(
        pos_indices, size=size, replace=len(pos_indices) < size
    )
    sampled_neg_indices = np.random.choice(
        neg_indices, size=size, replace=len(neg_indices) < size
    )
...
def default_collate(batch):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        if elem_type.__name__ == "ndarray" or elem_type.__name__ == "memmap":
            ...
            if np_str_obj_array_pattern.search(elem.dtype.str) is not None:
                raise TypeError(
                    default_collate_err_msg_format.format(elem.dtype)  # noqa
                )
    ...
    raise TypeError(default_collate_err_msg_format.format(elem_type))  # noqa
...
def calculate_positive_accuracy_by_time(positive_preds_dict):
    ...
    for i in range(len(time_dict)):
        ...
        return_list_cumulative.append(
            sum(time_dict_cumulative[str(i)])
            / len(time_dict_cumulative[str(i)])
        )
...
def calculate_time_to_detection_stats(positive_preds_dict, positive_times_dict):
    ...
    return (
        ...
        median_time_to_detection,
        std_time_to_detection,
    )
